...
#path=path("c:/home/robaking/Desktop/Voting_System-main/database")
path = Path("database")

def count_reset():
    df=pd.read_csv(path/'voterList.csv')
    df=df[['voter_id','Name','Gender','RegNo','Faculty','Passw','hasVoted']]
...
def reset_voter_list():
    df = pd.DataFrame(columns=['voter_id','Name','Gender','RegNo','Faculty','Passw','hasVoted'])
    df=df[['voter_id','Name','Gender','RegNo','Faculty','Passw','hasVoted']]
    df.to_csv(path/'voterList.csv')
...
def taking_data_voter(name,gender,RegNo,Faculty,passw):
    df=pd.read_csv(path/'voterList.csv')
    df=df[['voter_id','Name','Gender','RegNo','Faculty','Passw','hasVoted']]
    row,col=df.shape
    if row==0:
