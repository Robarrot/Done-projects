...
def Home(root, frame1, frame2):
    ...
    Button(frame2, text="Home",bg="light steel blue",command = lambda: Home(root, frame1, frame2)).grid(row=0,column=0)
    ...
    #Admin Login
    admin = Button(frame1, text="Admin Login",bg="light steel blue", width=15, command = lambda: AdmLogin(root, frame1))

    #Voter Login
    voter = Button(frame1, text="Voter Login",bg="light steel blue", width=15, command = lambda: voterLogin(root, frame1))

    #New Tab
    newTab = Button(frame1, text="New Window",bg="light steel blue", width=15, command = lambda: sb_p.call('start python homePage.py', shell=True))
