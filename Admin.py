...
def AdminHome(root,frame1,frame3):
    ...
    Button(frame3, text="Admin",bg="light steel blue", command = lambda: AdminHome(root, frame1, frame3)).grid(row = 1, column = 0)
    frame3.pack(side=TOP)

    Label(frame1, text="Admin",bg="steel blue",font=('Helvetica', 25, 'bold')).grid(row = 0, column = 1)
    Label(frame1, text="",bg="steel blue").grid(row = 1,column = 0)

    #Admin Login
    runServer = Button(frame1, text="Run Server",bg="light steel blue", width=15, command = lambda: os.system('python3 Server.py'))

    #Voter Login
    registerVoter = Button(frame1, text="Register Voter",bg="light steel blue", width=15, command = lambda: regV.Register(root, frame1))

    #Show Votes
    showVotes = Button(frame1, text="Show Votes",bg="light steel blue", width=15, command = lambda: adFunc.showVotes(root, frame1))
...
def log_admin(root,frame1,admin_ID,password):
    ...
    if(admin_ID=="Admin" and password=="admin"):
        ...
        msg = Message(frame1, text="Either ID or Password is Incorrect",bg="LightSteelBlue1", width=500)
...
def AdmLogin(root,frame1):
    ...
    Label(frame1, text="Admin Login",bg="steel blue", font=('Helvetica', 18, 'bold')).grid(row = 0, column = 2, rowspan=1)
    Label(frame1, text="",bg="steel blue").grid(row = 1,column = 0)
    ...
    sub = Button(frame1, text="Login", bg="light steel blue",width=10, command = lambda: log_admin(root, frame1, admin_ID.get(), password.get()))
    Label(frame1, text="",bg="steel blue").grid(row = 4,column = 0)
