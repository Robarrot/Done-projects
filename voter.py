...
def failed_return(root,frame1,client_socket,message):
    for widget in frame1.winfo_children():
        widget.destroy()
    message = message + "... \nTry again..."
    Label(frame1, text=message,bg="steel blue" ,font=('Helvetica', 12, 'bold')).grid(row = 1, column = 1)
    client_socket.close()
...
def voterLogin(root,frame1):
    ...
    Label(frame1, text="Voter Login",bg="steel blue", font=('Helvetica', 18, 'bold')).grid(row = 0, column = 2, rowspan=1)
    Label(frame1, text="",bg="steel blue").grid(row = 1,column = 0)
    Label(frame1, text="Voter ID:      ",bg="steel blue", anchor="e", justify=LEFT).grid(row = 2,column = 0)
    ...
    sub = Button(frame1, text="Login",bg="light steel blue", width=10, command = lambda: log_server(root, frame1, client_socket, voter_ID.get(), password.get()))
